...
def parse_audio_files(parent_dir,sub_dir_labels,file_ext="*.wav"):
    ...
    if features.size == 0 or labels.size == 0:
        raise Exception("no audio files detected to parse")
...
parent_dir = 'training2'


# array of directories - each dir is specific to a class -> [directory for class, numeric label for class]
# tr_sub_dirs_labels = [["air_conditioner_training",0], ["car_horn_training",1]]#, ["children_playing_training",2]] # training set
# ts_sub_dirs_labels = [["air_conditioner_testing",0], ["car_horn_testing",1]]#, ["children_playing_testing",2]] # test set
...
n_classes = 6 # len(np.unique(tr_labels)) # number of classes in training set (how many categories are we classifying)
...
with tf.Session() as sess:
    ...
    print("Test accuracy: {0:.3f}%".format(accuracy_output*100))
...
def visualize_results(y_pred, y_true):
    ...
    for c in set(y_true):
        ...
        bold = '\033[1m'
        reset = '\033[0m'
        result = "{0}Class {1}:{2} \n\t accuracy: {3:.2f}% \n\t most confused with: {4}".format(bold, c, reset, accuracy*100, most_confused_key)
