...
def extract_feature(file_name):
    ...
    default_sr = 22050
    X, sample_rate = librosa.load(file_name, sr=default_sr*5)
    stft = np.abs(librosa.stft(X))
    mfccs = np.amax(librosa.feature.mfcc(y=X, sr=sample_rate, n_mfcc=40).T,axis=0)
    chroma = np.amax(librosa.feature.chroma_stft(S=stft, sr=sample_rate).T,axis=0)
    mel = np.amax(librosa.feature.melspectrogram(X, sr=sample_rate).T,axis=0)
    # print(type(mel))
    # print(np.size(librosa.feature.melspectrogram(X, sr=sample_rate).T,axis=0))
    contrast = np.amax(librosa.feature.spectral_contrast(S=stft, sr=sample_rate).T,axis=0)
    tonnetz = np.amax(librosa.feature.tonnetz(y=librosa.effects.harmonic(X),
...
def parse_audio_files(parent_dir,sub_dir_labels,file_ext="*.wav"):
    ...
    for sub_dir, label in sub_dir_labels:
        for fn in glob.glob(os.path.join(parent_dir, sub_dir, file_ext)):
            ...
            ext_features = np.hstack(f)

            if features is None:
                features, labels = np.empty((0,len(ext_features))), np.empty(0, int)

            # ext_features = np.hstack([mfccs,chroma,mel,contrast,tonnetz])
            # ext_features = [mfccs,chroma,mel,contrast,tonnetz]
...
parent_dir = 'tapping_data'

# array of directories - each dir is specific to a class -> [directory for class, numeric label for class]
classes = ["glass_tapping", "wood_tapping", "metal_tapping", "tape_tapping"]
tr_sub_dirs_labels = [] # training set
ts_sub_dirs_labels = [] # test set
for i in range(len(classes)):
    tr_sub_dirs_labels.append(["{}_training".format(classes[i]), i])
    ts_sub_dirs_labels.append(["{}_testing".format(classes[i]), i])
...
n_classes = num_training_classes
n_hidden_units_one = 280 # ADJUST ACCORDINGLY default=280
n_hidden_units_two = 300 # ADJUST ACCORDINGLY default=300
sd = 1 / np.sqrt(n_dim)
learning_rate = 0.001 # ADJUST ACCORDINGLY (how quickly the models learns) default=0.01
...
def print_delim(num_times = 1, delimeter="~"):
...
with tf.Session() as sess:
    ...
    print_delim(2)
    print("Overall test accuracy: {0:.2f}%".format(accuracy_output*100))
    print_delim(2)
...
def visualize_results(y_pred, y_true):
    ...
    for c in set(y_true):
        ...
        bold = '\033[1m'
        reset = '\033[0m'
        result = "{0}{1}:{2} \n\t accuracy: {3:.2f}% \n\t most confused with: {4}".format(bold, classes[c], reset, accuracy*100, most_confused_key)
